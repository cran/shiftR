...
shiftrPrepareRight = function(set){
    ...
    for( i in 0:31 ) {
        # cat('i',i,'\n')
        rez[[i+1]] = packBits(
                        x = c(rep(zero, i), set, rep(zero, (-i-n) %% 32)),
                        type = 'integer');
    }
    class(rez) = "fcpRight";
    attr(rez, "sum") = sum(set);
...
singlePermutation = function(left, right, offset){
    ...
    .Call("CbitSumAndYinX", left, right[[1 + (offset) %% 32]], offset %/% 32,
            PACKAGE = "shiftR")
}
...
shiftrPermBinary = function(
    ...
    if(length(offsets)>0L){
        ...
        for( i in seq_along(offsets) ) 
            overlapsPerm[i] = .Call("CbitSumAndYinX", 
                                    left, 
                                    right[[1 + (offsets[i]) %% 32]],
                                    offsets[i] %/% 32,
                                    PACKAGE = "shiftR");
